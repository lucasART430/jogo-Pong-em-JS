...
class Bola {
    ...
    checarBordas() { // sendo igual a zero quer dizer que encostou em uma extremidade
        if (this.posicaoX - this.diametro / 2 <= 0 || this.largura == this.diametro) { // checa se a bola encosta no lado esquerdo
            this.jogo.pontuar(2);
            this.jogo.parar();
        } else if (this.posicaoX + this.diametro / 2 >= width || this.largura == this.diametro) { // checa se a bola encosta no lado direito
            this.jogo.pontuar(1);
            this.jogo.parar();
        }
...
class Jogador {
    ...
    movimentar() {
        //movimentos do jogador 1 para cima e baixo (esquerda)
        if (this.id == 1) {
            if (keyIsDown(87)) {
                if (this.posicaoY > 0) {
                    this.posicaoY -= this.velocidadeY;
            ...
            let velocidadeAleatoria = random(-13, -12, -11, -9, 9, 11, 12, 13);
            this.posicaoY += (bola1.posicaoY - this.posicaoY) * 0.1 + velocidadeAleatoria;
...
function draw() { // draw - desenhar. Roda em loop, serve para desenhar na tela, tudo que colocar na função será executado até o programa terminar
    ...
    document.addEventListener('keydown', function (event) {
        if (event.key === 'm' || event.key === 'M') {
            window.location.href = "/jogoPongPgnPrincipal.html"; // Substitua "/" pelo caminho completo da sua página principal
